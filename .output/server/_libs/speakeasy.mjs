...
import { t as require_base32_js } from "./base32.js.mjs";
//#region node_modules/.pnpm/speakeasy@2.0.0/node_modules/speakeasy/index.js
var require_speakeasy = /* @__PURE__ */ __commonJSMin(((exports) => {
	var base32 = require_base32_js();
	...
	*/
	exports.digest = function digest(options) {
		...
		if (!Buffer.isBuffer(secret)) secret = encoding === "base32" ? base32.decode(secret) : new Buffer(secret, encoding);
	...
	*/
	exports.hotp.verifyDelta = function hotpVerifyDelta(options) {
		...
		for (i = counter; i <= counter + window; ++i) {
			options.counter = i;
			if (parseInt(exports.hotp(options), 10) === token) return { delta: i - counter };
		}
	};
	/**
	...
	*/
	exports.generateSecret = function generateSecret(options) {
		...
		var symbols = true;
		if (options.symbols !== void 0 && options.symbols === false) symbols = false;
		...
		if (otpauth_url) SecretKey.otpauth_url = exports.otpauthURL({
	...
	*/
	exports.generateSecretASCII = function generateSecretASCII(length, symbols) {
		...
		if (symbols) set += "!@#$%^&*()<>?/[]{},.:;";
		var output = "";
		for (var i = 0, l = bytes.length; i < l; i++) output += set[Math.floor(bytes[i] / 255 * (set.length - 1))];
		return output;
	};
	...
	*/
	exports.otpauthURL = function otpauthURL(options) {
		...
		switch (type) {
			case "totp":
			case "hotp": break;
			default: throw new Error("Speakeasy - otpauthURL - Invalid type `" + type + "`; must be `hotp` or `totp`");
		}
		if (!secret) throw new Error("Speakeasy - otpauthURL - Missing secret");
		if (!label) throw new Error("Speakeasy - otpauthURL - Missing label");
		if (type === "hotp" && (counter === null || typeof counter === "undefined")) throw new Error("Speakeasy - otpauthURL - Missing counter value for HOTP");
		...
		if (algorithm != null) {
			switch (algorithm.toUpperCase()) {
				...
				case "SHA256":
				case "SHA512": break;
				default: console.warn("Speakeasy - otpauthURL - Warning - Algorithm generally should be SHA1, SHA256, or SHA512");
			}
			query.algorithm = algorithm.toUpperCase();
		}
		if (digits != null) {
			if (isNaN(digits)) throw new Error("Speakeasy - otpauthURL - Invalid digits `" + digits + "`");
			else switch (parseInt(digits, 10)) {
				case 6:
				case 8: break;
				default: console.warn("Speakeasy - otpauthURL - Warning - Digits generally should be either 6 or 8");
			}
			query.digits = digits;
		}
		if (period != null) {
			period = parseInt(period, 10);
			if (~~period !== period) throw new Error("Speakeasy - otpauthURL - Invalid period `" + period + "`");
			query.period = period;
		}
		return url.format({
			protocol: "otpauth",
